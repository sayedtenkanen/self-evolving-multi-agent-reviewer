...
class TrajectoryStore:
    ...
    async def get_agent_stats(self, agent_id: str) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Total reviews
            total = conn.execute(
                "SELECT COUNT(*) as count FROM trajectories WHERE agent_id = ?",
                (agent_id,)
            ).fetchone()["count"]

            # Reviews by language
            by_language = conn.execute(
                "SELECT language, COUNT(*) as count FROM trajectories WHERE agent_id = ? GROUP BY language",
                (agent_id,)
            ).fetchall()

            # Recent activity
            recent = conn.execute(
                "SELECT * FROM trajectories WHERE agent_id = ? ORDER BY timestamp DESC LIMIT 10",
                (agent_id,)
            ).fetchall()
